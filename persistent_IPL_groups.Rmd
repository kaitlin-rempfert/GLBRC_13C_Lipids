...
```{r}
#merge with metadata
e_data_long_Rodin <- left_join(e_data_long_intensity, lipidomics_Rodin_meta) %>% na.omit()
...
e_data_long_meta <- e_data_longer_Rodin_g %>% select(c(`Lipid Group`, Site, TimePoint, id, group, log2_intensity_group)) %>% dplyr::rename("Compound" = "Lipid Group", "log2_intensity" = "log2_intensity_group") %>% distinct()
```
...
all_classified_summarized <- all_classified %>% 
  ...
  select(c(Compound, group, atomperc_avg, log2_intensity_avg, log2FC_intensity, atomperc_timepoint_avg_1year, p_enrich, effsize_enrich, p_intensity, effsize_intensity, classification, site_classification)) %>% distinct()
...
all_classified_sep_summarized <- all_classified_sep %>% 
  ...
  fill(atomperc_timepoint_avg_2month, .direction = "up") %>%
  mutate(log2FC_intensity = intensity_timepoint_avg_2month - intensity_timepoint_avg_1year) %>%
  select(c(Compound, group, Site, atomperc_avg, log2_intensity_avg, log2FC_intensity, atomperc_timepoint_avg_1year, p_enrich, effsize_enrich, p_intensity, effsize_intensity, classification, site_classification)) %>% distinct()
```
...
lipids_wmeta_ordered <- all_classified_summarized %>% select(c(Compound, group, classification, atomperc_avg, atomperc_timepoint_avg_1year, p_enrich, effsize_enrich, log2_intensity_avg, p_intensity, effsize_intensity, log2FC_intensity, site_classification)) %>% 
...
#sep
#reorder data (MI switchgrass 2 month, MI switchgrass 1 year...then WI with same pattern)
lipids_wmeta_ordered_sep <- all_classified_sep_summarized %>% select(c(Compound, group, classification, Site, atomperc_avg, atomperc_timepoint_avg_1year, p_enrich, effsize_enrich, log2_intensity_avg, p_intensity, effsize_intensity, log2FC_intensity, site_classification)) %>% 
...
draw(ha_empty + ha_class + ha_lipid + h_atmperc2 + ha1 + ha2 + ha3 + ha4 + ha5, annotation_legend_list = list(lgd_lipid, lgd_pvalue, lgd_sig), heatmap_legend_side = "right", annotation_legend_side = "right", merge_legends = TRUE)


for(i in 1:4) {
    decorate_annotation("foo", slice = i, {
        grid.text(paste(text_list[[i]], collapse = "\n"), x = unit(5, "mm"), rot = 0, just = "right")
    })
}
```
