...
```{r}
p <- all_meta %>% filter(timepoint == "2month") %>% filter(sig == "significant") %>% ggboxplot(x = "lipid_class", y = "mean_rel_enrichment",
      color = "lipid_class", palette = "Paired") + rremove("legend") + labs(x = "Lipid Class", y = expression("Excess Atom Fraction "*{}^13*"C"))
p

bp <- readRDS("output/bp.rds")

plot_grid(bp, p, labels = c("A", "B"), ncol = 1)
```
